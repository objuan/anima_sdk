...
Anima::ProgressBar *BulkGUIManager::OpenProgressBar(const QString &title,int phaseCount,bool openwin)
{
	...
	if (openwin)
		GetCOREInterface()->ProgressStart (_QM(QString("Anima: %1").arg(title)),true,&OpenProgressBar_FN,bar);
	return bar;
}
...
void BulkGUIManager::UpdateProgressBar(Anima::ProgressBar *bar)
{
	if (bar->openwin)
	{
		...
		GetCOREInterface()->ProgressUpdate(bar->PercentPhase(),false,_QM(desc));
	}
}
...
//public:
//	const MCHAR *GetName() { return _M("Anima"); }
//	HWND hwnd;
...
//		QApplication app(argc, argv);
  		...
  		try
		{
			...
			try
			{
				...
				{
					QString path;
					path = QString("%1/Anima.xml").arg(_MQ(mgr->GetPlugInDir(i)));
			...
			else
			{
				...
				Anima::GfxContext::Singleton->OpenWIN(0,1,1); //Ogre::Root::getSingletonPtr()->createRenderWindow("AnimaWindow", 1,1, false, NULL);
				renderWindow = Anima::GfxContext::Singleton->getRenderWindow();
				//	renderWindow=NULL;
