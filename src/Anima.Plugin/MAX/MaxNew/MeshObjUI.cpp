...
void UpdateLabelInfo()
{
	...
	if(hwndLblInfo!=NULL)
	{
		...
		MSTR str = _QM(desc);
		//SendMessage(hwndLblInfo, WM_SETTEXT, (WPARAM) str.ToMCHAR(), 0);
		SendMessage(hwndLblInfo, WM_SETTEXT, 0, (LPARAM)  str.ToMCHAR());
	}
}
void UpdateFileNameInfo()
{
	...
	if(hwndLblFileName!=NULL)
	{
		...
		MSTR str = _QM(name);
		SendMessage(hwndLblFileName, WM_SETTEXT, 0, (LPARAM)  str.ToMCHAR());
	}
}
...
void OnDlgCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	...
	else if (id == IDC_BUTTON_RELOAD)
	{
		...
		//MeshObject::meshObject->LoadFromGUI(MeshObject::meshObject->animaPath,FindNodeRef(MeshObject::meshObject));
			...
			else
			{
				...
				MessageBox(NULL, _QM(QString("The path to the original .ani scene has changed (%1) .\r\n\ Please use the Browser window to navigate and locate the new file.").arg(MeshObject::meshObject->animaPath)), _T("WARNING!"), MB_OK);
				load=true;
			}
			UpdateLabelInfo();
		}
	...
	if (load)
	{
		...
		wcscpy (szFileName,_QM(MeshObject::meshObject->animaPath));
		...
		ofn.hwndOwner = hwnd;
		ofn.lpstrFilter = _M("AXYZ Anima Files (*.ani)\0*.ani\0All Files (*.*)\0*.*\0");
		ofn.lpstrFile = szFileName;//_QM(QString("%1").arg(szFileName));
		ofn.nMaxFile = MAX_PATH;
		ofn.lpstrTitle		=  _M("Load a 'Anima' File");
		ofn.Flags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY ;//|OFN_ALLOWMULTISELECT;
		ofn.lpstrDefExt =  _M("ani");
		...
		if(GetOpenFileName(&ofn))
		{
			...
			MeshObject::meshObject->Load(_MQ(szFileName),FindNodeRef(MeshObject::meshObject),true,true);
