...
MSTR _QM(const QString &qstr) 
{
	 if (qstr.length() > 0)
    {
        return (const wchar_t*)qstr.unicode();
    }
    else
    {
        return _M("");
...
{
	return QString::fromUtf8(x.ToUTF8());
	//return QString::fromWCharArray(x);
}
...
void MeshObject::Clear()
{
	...
	TSTR undostr; undostr.printf(_M("Select"));
...
//	lastUpdateTime= SecToTicks(999999999999);
	...
	try
	{
		...
		else
		{
			...
			//	{
			//		// avviso attesa
			//		MessageBox(NULL, _M("The .ani scene file you are trying to load does not have any collision data saved.\r\n\If you proceed, collisions are going to be calculated while loading.\r\n\This process may take some time."), _M("WARNING!"), MB_OK);
			//		//GetCOREInterface()->DisplayTempPrompt("This file don't have collisions, please wait for rebuild");
			//	}
			//	delete fileInfo;
			//}
			//catch(...)
			//{
			//	//MessageBox(NULL, QString("Cannot load .ani scene : %1").arg(animaPath).toAscii().data(), _M("WARNING!"), MB_OK);
			//	MessageBox(NULL, _M("Cannot load .ani scene : %1"), _M("WARNING!"), MB_OK);
			//}
			ok = contextIO.Load(animaPath,false,progressBar); // carico nell'ambiente
		}
