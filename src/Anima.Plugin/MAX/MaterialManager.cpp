...
QString map[] = {"Default Scanline Renderer","STANDARD","mental ray Renderer","MENTAL RAY","vray Renderer","VRAY","vray","VRAY","v-ray","VRAY","iray Renderer","IRAY","Nvidia Mental Ray","NVIDIA"};
...
Texmap* MakeTex(const QString &name) {
	...
	//bmt->SetMapName
	bmt->SetMapName(name.toAscii().data());
	...
	for(int s=0;s<m->NumSubTexmaps ();s++)
	{
		...
		if (txt!=NULL)
		{
			...
			if (sname ==MSTR("Bitmap"))
			{
				...
				TCHAR *path= btm->GetMapName();

				// controllo se ce'
				QString filePath = QString("%1").arg(path);
				if (!QFile(filePath).exists())
				{
					...
					if (QFile(filePath).exists())
					{
						btm->SetMapName(filePath.toAscii().data());
					}
				}
			}
	...
	for(int s=0;s<num;s++)
	{
		...
		if (txt!=NULL)
		{
			...
			//if (slot_name == MSTR("Diffuse") )
			{
				...
				else if (sname ==MSTR("VRayCompTex"))
				{
					ChangeRgb(mi,mat,txt,newValue,0);
					maskPos=-1;
				}
				else if (sname ==MSTR("Normal Bump"))
				{
					ChangeRgb(mi,mat,txt,newValue,0);
				}
				else if (sname ==MSTR("Mask"))
				{
					// Texmap
...
Mtl *LoadMaterial(int *base_m_idx,MultiMtl *objMaterial,Anima::MeshExt *meshExt,Anima::ModelExporter *modelExporter,Anima::ModelExporterRender *modelExporterRender,const QString &materialName,int materialNameIdx)
{
	...
	if (expMat!=NULL)
	{
		...
		int ret = globalInt->LoadMaterialLib(MSTR(path.toAscii().data()));
		...
		int idx = base_lib.FindMtlByName(MSTR(matName.toAscii().data())); 
		if (idx != -1)
		{
		...
		else
		{
			...
			StdMat *m = New3DSDefaultMtl(name.toAscii().constData(),"","",-1,NULL);
			//loadMtls.AddMtl(m);			
			ret_mat = m;
...
void MeshObject::ClearAnimaMaterial(){
	...
	do
	{
		...
		idx = lib->FindMtlByName(MSTR(name.toAscii().constData())); // TODO BETTER
		if (idx != -1)
...
//	MtlBaseLib &lib = Interface::GetMaterialLibrary()  
	...
	if (!Anima::Context::Singleton.GetPresetsExporterManager()->HaveSingleMaterial())
	{
		...
		for(int i=0;i<materialList.count();i++)
		{
			if (materialList[i].materialNumber != matScanIdx)
			{
				...
				mtl->SetName(name.toAscii().constData());
				...
				if(Anima::Context::Singleton.GetPresetsExporterManager()->GetViewportMode() != Anima::PresetsExporter_ViewportMode_TEXTURED)
				{
					...
					bakeshell->SetName("an(i)maMaterial");
		...
		//	Anima::_LogDebug(QString("model %1 %2").arg(materialList[i].materialNumber).arg(materialList[i].materialSubIdx),__FUNCTION__,__LINE__);
			...
			GetCOREInterface()->ProgressUpdate((float)i*100.0/(float)meshList.InstanceCount(),false,"Building Materials");
		...
		//	Anima::ModelExporter *_modelExporter = expPars->GetExporter(Anima::ModelExporterType::MAX,GetMaxVersion());
			...
			for(int t=0;t<matCount;t++)
			{
				...
				QString matName = meshExt->getMaterialName(t);
				// ho i dati del materiale di ogre

				if (modelExporterRender!=NULL && qualityInfo->Folder !="")
				{
					Anima::ModelExporterRenderMaterial *expMat = modelExporterRender->FindMatBySource(matName);
					if (expMat!=NULL)
					{
						...
						
						{
							...
							else
							{
								...
								StdMat *m = New3DSDefaultMtl(name.toAscii().constData(),"","",t,&subInfo);
								loadMtls.AddMtl(m);			
		...
		if (_modelExporter!= NULL)
		{
			...
			if (expMat!=NULL)
			{
				...
				//	QString name = QString("%1_%2").arg(meshExt->GetModel()->GetModel()->GetName()).arg(partName);
					...
					StdMat *m = New3DSDefaultMtl(name.toAscii().constData(),"","",0,NULL);
					loadMtls.AddMtl(m);			
	...
	objMaterialExtra->SetName(name.toAscii().constData());
