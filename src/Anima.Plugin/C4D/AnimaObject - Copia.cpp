...
float AnimaObject::g_endTime = MAXREALs;
...
void AnimaObject::GetDimension(BaseObject* op, Vector* mp, Vector* rad)
{
	*mp	 = Vector(0.0);
	*rad = Vector(100.0);
}

DRAWRESULT AnimaObject::Draw(BaseObject* op, DRAWPASS drawpass, BaseDraw* bd, BaseDrawHelp* bh)
{
...
Bool AnimaObject::Init(GeListNode *node)
{
	...
	data->SetLong(ANIMAOBJECT_BBOX_MODE, 100);
	m_bCached = FALSE;
	BaseObject::Free(m_obj);
	return TRUE;
}
...
Bool AnimaObject::GetDParameter(GeListNode* node, const DescID& id, GeData& t_data, DESCFLAGS_GET& flags)
{
	switch(id[0].id)
	{
		...
		case ANIMAOBJECT_SCENEINFO_ACTORS:
			flags |= DESCFLAGS_GET_PARAM_GET;
			t_data.SetString(QString("%1 of %2").arg(m_infos.shownActors).arg(m_infos.actors).toAscii().data());
			//t_data.SetString(LongToString(m_infos.shownActors) + " of " + LongToString(m_infos.actors));
			break;
		case ANIMAOBJECT_SCENEINFO_POLYS:
			flags |= DESCFLAGS_GET_PARAM_GET;
			t_data.SetString(QString("%1 of %2").arg(m_infos.shownPolys).arg(m_infos.polys).toAscii().data());
			//t_data.SetString(LongToString(m_infos.shownPolys) + " of " + LongToString(m_infos.polys));
			break;
		case ANIMAOBJECT_SCENEINFO_POINTS:
			flags |= DESCFLAGS_GET_PARAM_GET;
			t_data.SetString(QString("%1 of %2").arg(m_infos.shownVerts).arg(m_infos.verts).toAscii().data());
		...
		case ANIMAOBJECT_SCENEINFO_RENDERER:
			{
				...
				if(!rd)
					break;
				switch(rd->GetDataInstance()->GetLong(RDATA_RENDERENGINE) )
				{
...
Bool AnimaObject::Message(GeListNode *node, Int32 type, void *t_data)
{
	switch(type)
	{
		...
		case MSG_CHANGE:
		{
			...
			float curDarkness = ((BaseObject*)node)->GetDataInstance()->Getfloat(ANIMAOBJECT_DARKNESS);
...
void BuildMaterials(BaseDocument* doc, Anima::MeshExtCollection* meshList, Anima::GeomExtCollection* geomList)
{
	if(doc)
	{
		...
		for(Int32 i=0;i<meshList->InstanceCount();i++)
		{
			...
			{
				String stName = QString("%1%2").arg(ANIMA_MAT_PRE).arg(matIdx+t).toAscii().data();//ANIMA_MAT_PRE+LongToString(matIdx+t);
				...
				{
					expMat = modelExporterRender->FindMatBySource(subMesh->getMaterialName().c_str());
					expMatOnlyColor = modelExporterRenderOnlyColor->FindMatBySource(subMesh->getMaterialName().c_str());
					if(bSingleMaterial)
					...
					String matName = Utils::toC4DString(subMesh->getMaterialName().c_str());
					String matPrefix = "";
		...
		for(Int32 i=0;i<geomList->Count();i++)
		{
			...
			if (modelExporterRender!=NULL)
			{
				for(Int32 m = 0 ; m< ext->GetMaterialNameCount() ; m++)
				{
					...
					String stName = (QString("%1%2").arg(ANIMA_MAT_PRE).arg(matIdx+m).toAscii().data());
					Anima::RenderPresetInfo *qualityInfo =  Anima::Context::Singleton.GetRenderInfo()->CurrentPreset;
...
BaseObject* AnimaObject::BuildAll(const Filename& f, BaseObject* op, Bool bEmbedded, Bool bForceFromFile, BaseDocument* doc, Bool resetScaleFactor)
{
	...
	if((f.Content() && GeFExist(f)) || bForceFromFile || bEmbedded)
	{
		...
		float startTime = film.GetStartActiveTime();
		float endTime = film.GetEndActiveTime();
...
//		GePrint(floatToString(startTime) + " to " + floatToString(endTime));
		...
		float scaleFactor = op->GetDataInstance()->Getfloat(ANIMAOBJECT_SCALE_FACTOR);
...
Bool AnimaObject::hasToShowBBox()
{
	...
	if(bc)
	{
		Int32 maxbbox = bc->GetLong(ANIMAOBJECT_BBOX_MODE);
